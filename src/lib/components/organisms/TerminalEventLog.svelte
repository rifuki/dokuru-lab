...
<script lang="ts">
	...
	function asCurl(event: TerminalEvent): string {
		...
		const url = `https://${hostname}${event.path}`;
	...
	function copyJson(event: TerminalEvent): string {
		if (event.kind === 'http') return JSON.stringify(event.response ?? { ok: false, error: event.error }, null, 2);
		if (event.kind === 'shell') return event.lines.map((l) => l.text).join('');
		return event.text;
	}
	...
	const filteredEvents = $derived(
		filter.trim()
			? events.filter((e) => {
					...
					if (e.kind === 'http') return `${e.method} ${e.path}`.toLowerCase().includes(needle);
					if (e.kind === 'shell') return (e.command || '').toLowerCase().includes(needle);
					return (e.kind === 'system' ? e.text : '').toLowerCase().includes(needle);
				})
			: events
	);
...
<div class="flex h-full min-h-0 flex-col bg-[#0a0a0a] text-white">
	...
	<header class="flex h-9 shrink-0 items-center gap-1.5 border-b border-white/5 px-3">
		<span class="inline-flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-[0.1em] text-white/55">
			<TerminalIcon size={11} strokeWidth={2.2} />
			Terminal
		</span>
		<span class="ml-1 rounded-full bg-white/[0.06] px-1.5 py-0.5 font-mono text-[9.5px] tabular-nums text-white/50">
			{events.length}
		</span>
		<span
			...
			<div class="mb-1 rounded-md border border-white/[0.04] bg-white/[0.015] text-white/80 transition-colors hover:border-white/10">
				...
				>
					...
					{:else if event.kind === 'shell'}
						...
						{/if}
					{:else}
						<span class={`truncate font-mono text-[11px] ${event.level === 'error' ? 'text-commerce' : event.level === 'warn' ? 'text-amber-300' : 'text-white/60'}`}>
							{event.text}
						</span>
					{/if}
				</button>
				...
				{#if open}
					<div class="border-t border-white/5 bg-black/30 px-3 py-2">
						{#if event.kind === 'http'}
							{#if event.body !== undefined}
								<div class="mb-2">
									<span class="mb-1 block font-mono text-[9.5px] uppercase tracking-[0.1em] text-white/40">Request body</span>
							...
							{:else}
								<div class="mb-2">
									<div class="mb-1 flex items-center justify-between">
										<span class="font-mono text-[9.5px] uppercase tracking-[0.1em] text-white/40">Response</span>
