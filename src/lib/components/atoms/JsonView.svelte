	...
	function highlightJson(text: string): string {
		return text.replace(
			...
			(match, stringOrKey, colon, bool, number) => {
				if (stringOrKey) {
					if (colon) return `<span class="text-playstation-cyan">${escapeHtml(stringOrKey)}</span>${colon}`;
					return `<span class="text-emerald-400">${escapeHtml(stringOrKey)}</span>`;
				}
				if (bool) return `<span class="text-[#c9a7ff]">${bool}</span>`;
				if (number) return `<span class="text-[#ffd479]">${number}</span>`;
				return match;
			}
		);
	}
